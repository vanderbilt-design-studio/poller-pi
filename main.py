...
async def send_status():
    async with websockets.connect(server_uri, ssl=ssl_context) as websocket:
        while True:
            ...
            status_json_dict = {'printers': listener.printer_jsons(), 'sign': sign_pins.as_value_dict(), 'key': X_API_KEY}
    ...
    while True:
        try:
            ...
            if type(serr) is KeyboardInterrupt:
                raise serr
            logging.warning(f"Exception while sending status, attempting to connect and send again: {serr}")
finally:
    zeroconf.close()
