...
class PrinterListener:
    def __init__(self):
        ...
        self.credentials_dict: CredentialsDict = CredentialsDict(credentials_filename=ultimaker_credentials_filename)
    ...
    def add_service(self, zeroconf, type, name):
        ...
        if len(info.addresses) == 0:
            ...
            return
        printer = Printer(socket.inet_ntoa(info.addresses[0]), info.port, Identity(ultimaker_application_name, ultimaker_user_name))
    ...
    def printer_jsons(self) -> List[Dict[str, str]]:
        ...
        for printer in list(self.printers_by_name.values()):
            try:
                ...
                if printer.credentials is not None and printer.get_system_guid() not in self.credentials_dict:
